...
ngx_int_t dav_next_copy_move_handler(ngx_http_request_t *r, dav_next_loc_conf_t *dlcf)
{
    ...
    } else { // SSL or no SSL scheme (in case of TLS termination in reverse proxy)
...
destination_done:; // HACK: ';' to avoid 'expected expression' error in clangd
...
overwrite_done:; // HACK: ';' to avoid 'expected expression' error in clangd
