...
ngx_int_t dav_next_propfind(ngx_http_request_t *r, ngx_uint_t props)
{
    ...
    // check if only main entry is needed and not at virtual root
    if (!ctx->is_virtual_root && (depth == 0 || !main_entry->dir)) {
        // send immediate response in that case
    ...
    if (ctx->is_virtual_root) {
        ...
        *last++ = '/';
        ...
        if (ctx->identity != NULL && ctx->identity->gids != NULL) {
            for (ngx_int_t i = 0; i < (ngx_int_t) ctx->identity->gids->nelts; i++) {

                // take group name
                name = ((ngx_str_t *) ctx->identity->gids->elts)[i];
                ...
                *last++ = '/';
                ...
                // main_entry time is the most recent
                time_t entry_mtime = ngx_dav_next_file_mtime(&fi);
                if (main_entry->mtime < entry_mtime) {
                    main_entry->mtime = entry_mtime;
                }
    ...
    for ( ;; ) {
        ...
        if (!dir.valid_info) {
            ...
            *last++ = '/';
            ...
            DEBUG1(r->connection->log, ngx_errno, ngx_de_info_n " '%s' tested", filename);
        ...
        entry->name.data = ngx_pstrdup(r->pool, &name);
        RETURN_500_IF(entry->name.data == NULL);
