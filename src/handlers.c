...
{
    ngx_str_t null_uri = ngx_null_string;
    ...
    if (new_uri.len >= 1 + path_offset + 1 + r->headers_in.user.len &&
        ...
        DEBUG1(r->connection->log, 0, "DEBUG: path2222222 to CREATE_DIR=%V (user dir)", &path);
    ...
    // check if we have provides for groups
    dav_next_loc_conf_t *dlcf = ngx_http_get_module_loc_conf(r, dav_next_module);
    if (dlcf->auth_provider == NULL) {
        DEBUG0(r->connection->log, 0, "no auth providers → no GROUP");

        // if not, ignore groups and return bad news
        return null_uri;
    }

    if (ctx->identity == NULL) {
        if ((ctx->identity = dav_next_auth_get_identity(r)) == NULL) {
            return null_uri;
        }
    }

    if (ctx->identity->gids == NULL) {
        DEBUG0(r->connection->log, 0, "ctx->identity->gids = NULL");
        return null_uri;
    }

    for (ngx_uint_t i = 0; i < ctx->identity->gids->nelts; i++) {
        ngx_str_t *group = (ngx_str_t *) ctx->identity->gids->elts + i;

        DEBUG3(r->connection->log, 0, "TRYING %V %d/%d?", group, new_uri.len, path_offset + group->len);

        // if the original URI was under /<user>/<group>/
        if (new_uri.len >= 1 + path_offset + 1 + group->len &&
            ngx_strncmp(group->data, new_uri.data + 1 + path_offset + 1,
                        group->len) == 0 &&
            ...
            // create the root dir, of fail silently if it already exists
            if (ngx_create_dir(path.data, 0770) == NGX_FILE_ERROR &&
...
ngx_int_t dav_next_preaccess_handler(ngx_http_request_t *r)
{
    ...
    if (ctx->access_needed == 0) {
        return dlcf->auth_provider->disable(r, dlcf);
    }
...
ngx_int_t dav_next_precontent_handler(ngx_http_request_t *r)
{
    ...
    // no need to be auth'ed
    if (ctx->access_needed == 0) {
        RETURN_RC_IF_NOK(dlcf->auth_provider->enable(r, dlcf));
    }
    ...
    if (ctx->alias != 0) {
        ...
        ctx->rewritten->data = ngx_pstrdup(r->pool, &save_uri);
    ...
    if (r->method == NGX_HTTP_GET) {
        ...
        ngx_table_elt_t *oc_fileid = ngx_list_push(&r->headers_out.headers);
        RETURN_500_IF(oc_fileid == NULL);

        oc_fileid->hash = 1;
        ngx_str_set(&oc_fileid->key, "OC-FileID");
        ...
        oc_fileid->value.len = 8 + DAV_NEXT_HEX_ID_LEN;
        oc_fileid->value.data = p;

        ngx_table_elt_t *fileid = ngx_list_push(&r->headers_out.headers);
        RETURN_500_IF(fileid == NULL);

        fileid->hash = 1;
        ngx_str_set(&fileid->key, "id");
...
void dav_next_put_handler(ngx_http_request_t *r)
{
    ...
    if (date_etag > 0 && date > 0 && date_etag == (uint64_t) date * _1G) {
        date_etag++;
